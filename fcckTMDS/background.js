...
chrome.runtime.onMessage.addListener(
  function(request, sender, sendResponse) {
    ...
    console.log(request);
    */
    if (request.getInitStat) {
      console.log("isInit = " + isInit);
      chrome.tabs.query({active: true}, function(tabs) {
  			//console.log(tabs[0]);
		  	chrome.tabs.sendMessage(tabs[0].id, {isInit: isInit}, function(response) {
		  });
		});
  		...
  		chrome.tabs.query({active: true}, function(tabs) {
  			//console.log(tabs[0]);
		  	chrome.tabs.sendMessage(tabs[0].id, {start: true, currentTerr: selectedTerr[indexTerr]}, function(response) {
		  });
		});
  	} else if (request.finishTerr) {
  		if (indexTerr >= selectedTerr.length) {
  			chrome.tabs.query({active: false}, function(tabs) {
			  chrome.tabs.sendMessage(tabs[1].id, {finishAll: true}, function(response) {
			  });
			});
  		} else {
  			//indexTerr++;
  			//chrome.tabs.query({active: false}, function(tabs) {
			  //     chrome.tabs.sendMessage(tabs[1].id, {currentTerr: selectedTerr[indexTerr]}, function(response) {});
			  // });
  		}
